...
function ActaForm() {
    ...
    const fetchEquiposDisponibles = useCallback(async () => {
        try {
            const response = await fetch('http://localhost:8080/EquipoServlet?accion=listar&estado=disponible');
            if (!response.ok) {
    ...
    const handleSubmit = (e) => {
        ...
        fetch('http://localhost:8080/ActasServlet', {
            method: 'POST',
    ...
    const consultarActas = () => {
        fetch(`http://localhost:8080/ActasServlet?cedula=${consulta}`)
            .then(res => res.json())
    ...
    const descargarPDF = (cedula) => {
        fetch(`http://localhost:8080/DescargarPDFServlet?cedula=${cedula}`)
            .then(response => {
