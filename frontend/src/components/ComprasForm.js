...
const ComprasForm = () => {
    ...
    const fetchSolicitudes = useCallback(async () => {
        ...
        try {
            ...
            const url = `http://localhost:8080/listar-solicitudes-compra?${queryParams.toString()}`;
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch('http://localhost:8080/solicitudes-compra', {
                method: 'POST',
                headers: {
    ...
    const handleEditSubmit = async (event) => {
        ...
        const url = `http://localhost:8080/solicitudes-compra/${solicitudToEdit.id}`;
    ...
    const handleDelete = async (solicitudId) => {
        ...
        if (window.confirm('¿Estás seguro de que quieres eliminar esta solicitud?')) {
            const url = `http://localhost:8080/solicitudes-compra/${solicitudId}`;
            try {
                const response = await fetch(url, {
                    method: 'DELETE',
                    credentials: 'include'
